...
public class AdminCourseService {
    ...
    private CourseDetailDto toDetailDto(Course course) {
        List<CourseDetailDto.ChapterDto> chapters = chapterRepository.findByCourseIdOrderByPosition(course.getId()).stream()
            .map(chapter -> CourseDetailDto.ChapterDto.builder()
                .id(chapter.getId())
                .title(chapter.getTitle())
                .position(chapter.getPosition())
                .lessons(lessonRepository.findByChapterIdOrderByPosition(chapter.getId()).stream()
                    .map(lesson -> CourseDetailDto.LessonDto.builder()
                        .id(lesson.getId())
